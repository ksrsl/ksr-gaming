...
const games: Game[] = [
  { id: "soccar", name: "SocCar", genre: "Sports", engine: "Unity", multiplayer: true, cover: "./games/SocCarCoverArt500.webp", background: "./games/SocCar.webp", iframe: "https://games.crazygames.com/en_US/soccar/index.html", description: "High-speed arena football with boost, jumps and mounted weapons.", controls: "WASD / ARROWS · MOUSE · SHIFT · SPACE" },
  { id: "prison-escape-lnj", name: "Prison Escape", genre: "Adventure", engine: "HTML5", multiplayer: false, cover: "./games/PrisonEscapeCoverArt500.webp", background: "./games/PrisonEscape.webp", iframe: "https://games.crazygames.com/en_US/prison-escape-lnj/index.html", description: "Outsmart the guards, solve challenges and make your way to freedom.", controls: "WASD · E · SPACE · MOUSE" },
  { id: "super-star-car", name: "Super Star Car", genre: "Racing", engine: "Unity", multiplayer: false, cover: "./games/SuperStarCarCoverArt500.webp", background: "./games/SuperStarCar.webp", iframe: "https://games.crazygames.com/en_US/super-star-car/index.html", description: "Build a racing career across precision circuits and upgrade your machine.", controls: "W / UP · A D / ARROWS · C · SPACE" },
  { id: "riders-downhill-racing", name: "Riders: Downhill", genre: "Racing", engine: "Unity", multiplayer: true, cover: "./games/RidersDownhillCoverArt500.webp", background: "./games/RidersDownhill.webp", iframe: "https://games.crazygames.com/en_US/riders-downhill-racing/index.html", description: "Race bikes, ATVs and speed boats through downhill stunt courses.", controls: "WASD / ARROWS · Q · E · SHIFT" },
  { id: "skillwarz", name: "SkillWarz", genre: "Shooter", engine: "Unity", multiplayer: true, cover: "./games/SkillWarzCoverArt500.webp", background: "./games/SkillWarz.webp", iframe: "https://games.crazygames.com/en_US/skillwarz/index.html", description: "A fast arena FPS with advanced movement and multiple competitive modes.", controls: "WASD · MOUSE · SPACE · SHIFT · R" },
  { id: "buildnow-gg", name: "BuildNow GG", genre: "Shooter", engine: "Unity", multiplayer: true, cover: "./games/BuildNowGGCoverArt500.webp", background: "./games/BuildNowGG.webp", iframe: "https://games.crazygames.com/en_US/buildnow-gg/index.html", description: "Build, aim and battle across competitive arenas and training modes.", controls: "WASD · MOUSE · SPACE · Q C V" },
  { id: "nzp", name: "Zombie Ops", genre: "Shooter", engine: "WebGL", multiplayer: true, cover: "./games/ZombieOpsCoverArt500.webp", background: "./games/ZombieOps.webp", iframe: "https://nzp.gay/", description: "A portable undead survival shooter built for instant browser play.", controls: "WASD · MOUSE · NUMBER KEYS" },
];
...
export default function KSRGaming() {
  ...
  useEffect(() => {
    ...
    const timers = skip ? [window.setTimeout(() => setBooting(false), 50)] : [window.setTimeout(() => setBootMessage(1), 1150), window.setTimeout(() => setBootMessage(2), 2200), window.setTimeout(() => setBootMessage(3), 3300), window.setTimeout(() => setBooting(false), 4400)];
    return () => { window.clearTimeout(savedStateTimer); timers.forEach((timer) => window.clearTimeout(timer)); };
  }, []);
  ...
  }, [view, selectedId, playingId, relay.connected, relay.isHost]);
  ...
  const powerCycle = () => {
    ...
    window.setTimeout(() => setBootMessage(1), 1150); window.setTimeout(() => setBootMessage(2), 2200); window.setTimeout(() => setBootMessage(3), 3300); window.setTimeout(() => setBooting(false), 4400);
  };
...
function GameStrip({ games: entries, selectedId, favorites, onSelect, onLaunch, onFavorite }: { games: Game[]; selectedId: string; favorites: string[]; onSelect: (id: string) => void; onLaunch: (game: Game) => void; onFavorite: (id: string) => void }) {
  return <div className="game-strip">{entries.map((game) => <GameCard key={game.id} game={game} selected={selectedId === game.id} favorite={favorites.includes(game.id)} onSelect={onSelect} onLaunch={onLaunch} onFavorite={onFavorite} wide />)}</div>;
}
...
function GameCard({ game, selected, favorite, onSelect, onLaunch, onFavorite, wide = false }: { game: Game; selected: boolean; favorite: boolean; onSelect: (id: string) => void; onLaunch: (game: Game) => void; onFavorite: (id: string) => void; wide?: boolean }) {
  return <article className={`game-card ${selected ? "selected" : ""} ${wide ? "wide" : ""}`} onMouseEnter={() => onSelect(game.id)}><button className="game-card-main" onClick={() => onSelect(game.id)} aria-label={`Select ${game.name}`} aria-current={selected ? "true" : undefined}><img src={wide ? game.background : game.cover} alt={game.name} /><span className="game-card-shade" /><span className="game-card-copy"><small>{game.genre}</small><strong>{game.name}</strong></span></button>{selected ? <button className="game-card-play" onClick={() => onLaunch(game)} aria-label={`Play ${game.name}`}>▶</button> : null}<button className={`game-card-favorite ${favorite ? "saved" : ""}`} onClick={() => onFavorite(game.id)} aria-label={`Favorite ${game.name}`}>{favorite ? "★" : "☆"}</button></article>;
}
